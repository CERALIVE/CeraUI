...
- ServerDialog protocol-first model [EXISTS]: `main/dialogs/ServerDialog.svelte` leads with `DestinationSection` (managed vs custom radiogroup), then the always-visible `ProtocolSelector` (protocol radiogroup, above the endpoint section — protocol-first reorder T21-T23), then shows `RelayServerSelector` (managed path) or `CustomEndpointForm` (custom path), then `TransportBadge` (now a read-only summary chip — no Advanced disclosure). The save path calls `buildServerSetConfig` and iterates its output for the field-lock loop, then calls `streaming.setConfig` once. When platform-managed ingest slots exist (T19), the managed path renders `server/ServerIngestSlots.svelte` (one-tap slot picker, `data-testid="ingest-slots"`) instead of `RelayServerSelector`, and the save path uses `buildManagedSlotConfig` for the resolved/picked slot.
- ServerDialog provider-sync [EXISTS]: `DestinationSection` labels the managed option using `config.remote_provider` (set by `CloudRemoteDialog`). If the operator switches provider without re-opening `ServerDialog`, the persisted `relay_server` may reference a server from the previous provider's relay list. The dialogs do NOT auto-clear `relay_server` (no silent mutation of operator config); instead the staleness is surfaced (T18). `isRelayServerStaleForProvider(relay_server, entries, provider)` (`receiver-experience.ts`) is the pure rule — a saved id absent from the catalog or tagged to a different managed cloud is stale; empty / untagged-legacy is never stale; callers MUST guard on `getRelays() !== undefined` so a still-loading list never false-warns. `CloudRemoteDialog` renders a `relay-provider-stale-warning` band when the chosen provider no longer owns the saved server; `ServerDialog` renders a `relay-stale-warning` band in the managed branch, plus a `relay-override-warning` band (`overrideClearsManagedBinding`) before save when a manual-endpoint override on a bound managed server would drop the `relay_server` binding. Copy: `settings.relayProviderSwitchWarning` / `settings.relayServerStale` / `settings.relayOverrideClearsBinding` (10 locales).
