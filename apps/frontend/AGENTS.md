...
- **…AND ONE GLYPH ANSWERS "how is this link's radio", WHICHEVER MODEM IS IN THE PORT [EXISTS]**: `buildBond` read a modem's strength from `modemSignal()` alone — i.e. from `status.signal` — and a `router-ethernet` dongle has no `status` block and never will. So `LinkSignal.signal` was `null` for it, `linkVisualState` fell to its "nothing was reported" arm, and the bond drew a bare muted `SignalZero` glyph beside a managed modem's spectral bar cluster: two treatments, one question, in one list. Operator-reported, and reproduced at 375/768/1280 — `L1 Quectel RM520N … 71%` with bars, `L2 ZTE MF79U · CELLULAR` with a dot. The dongle DOES publish a reading, on its own admin API, and the Cellular card has rendered it since Phase-C todo 21. `bondedSignalTier` (`lib/stores/hud/link-status.ts`) resolves it through **`resolveSignalInstrument` — the SAME single precedence rule**, never a second copy — onto the additive-optional `LinkSignal.signalTier`, and `LinkIndicator` maps a tier to the SAME three-bar cluster a percentage resolves to (`tierBarCount`, `lib/helpers/signal.ts`). Six properties are load-bearing. **A TIER IS CARRIED, NEVER A SYNTHESISED PERCENTAGE** — the cluster was always a three-bucket quantization, so a tier renders honestly in it, while the row's numeric readout stays absent rather than showing a figure nobody measured; the tier gets a WORD instead (`bonded-link-signal-tier`, reusing `network.cellular.signal.*`, so one reading is never named two ways). **A `device-stack` answer is never consumed here** — that branch is ModemManager's own and is already the percentage, so taking it would resurrect a tier for a modem `modemSignal` deliberately suppressed (a `no_sim` row, a negative sentinel). **An empty slot outranks every reading**, which is why `no_sim` moved above the tier arm in `linkVisualState`; with no tier passed the decision tree is byte-identical to the percentage-only rule it replaced, and a regression lock pins all eight states. **An `unknown` readout draws NO cluster** — an unreachable dongle, a refused session and a blank field are all "no reading was taken", and bars would report one. **FRESHNESS IS NOT A FILTER**: todo 20 re-serves one cycle's last live value precisely so a missed 30 s poll does not blank a row, and the Cellular card remains the surface that separates live from carried (it has the chip, the muted treatment and `data-freshness`); a three-bucket cluster never claimed recency. **The capability distinction is UNTOUCHED** — this unified a glyph, not the model: the Cellular row still draws `modem-router-signal` with its `Router` mark, dashed frame, `data-provenance` and prose sentence, and the two instruments still never share a row. Coverage: `signal.test.ts` (`tierBarCount` + the tier arm + the no-tier regression lock), `hud.test.ts` ("a self-managed dongle's own reading reaches the bond", including both never-resurrect controls), `BondedLinksSection.signalParity.test.ts` (rendered DOM, with the nothing-was-reported non-vacuity control), and `tests/e2e/visual/bonded-link-signal.visual.spec.ts` at 375/768/1280.
- **…AND THAT BADGE IS ONE COMPONENT, SHARED WITH THE HUD STRIP [EXISTS]**: `lib/components/custom/LinkBadge.svelte` owns the ordinal chip, the `--link-{n}` identity colour, the indicator call and (row variant) the name + kind/disambiguation column. `BondedLinksSection` and `HudBar`'s compact strip both compose it, because each previously carried its own inline copy and that is how the two came to disagree about which glyph a link gets. It renders its parts as SIBLINGS with no wrapper — every mount site is a flex row that already governs these boxes. TWO variants, and the difference between them is a product decision rather than drift: `row` carries the name and kind, `compact` carries ordinal + glyph ONLY, because the persistent strip's four-fact scope is deliberate — **do not let `compact` grow a name or a kind label**. Typography stays per-variant (the strip sets its ordinal in the mono face at its own size); what is shared is the structure, the colour and the glyph. Coverage: `LinkBadge.test.ts`, including the compact-variant two-fact lock.
