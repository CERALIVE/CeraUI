...
- **REMEMBER-ME STORES A TOKEN, NEVER THE PASSWORD [EXISTS]:** `localStorage.auth` holds the device-issued, individually REVOCABLE `auth_token` from a persistent login, and the reconnect path authenticates through `auth.login`'s `input.token` branch. It used to hold the operator's PLAINTEXT PASSWORD, resent as `input.password` on every reconnect — a permanent credential nobody could retire without changing the password everywhere. Four rules: `Auth.svelte` writes `message.auth_token` (an unremembered login writes NOTHING); `SystemHelper.savePassword` CLEARS the credential, because the device revokes every outstanding one on a password change and keeping a dead token only makes the next reconnect fail with no explanation; `Layout.svelte`'s clear-saved-session best-effort calls `revokePersistentToken` so the credential dies on the DEVICE too, un-awaited because the local clear is what the operator asked for; and sending the stored value as `password` would bcrypt-compare a token against the password hash and fail EVERY reconnect. Do not add a fourth writer of `localStorage.auth` without extending `src/tests/auth-no-plaintext-password.test.ts`, whose source leg exists to catch exactly that. Device half: [`../backend/AGENTS.md`](../backend/AGENTS.md) → A REMEMBER-ME CREDENTIAL IS A REVOCABLE TOKEN; policy: root charter **G3**.
- **Store ownership (single owner rule) [EXISTS]:** `lib/rpc/subscriptions.svelte.ts` is the SOLE `rpcClient.onMessage` consumer and owns every non-auth reactive/connection getter (`getConfig`, `getStatus`, `getIsConnected`, `getConnectionState`, …). `lib/stores/auth-status.svelte.ts` is the SOLE auth-mutation path (`ingestAuth`/`getAuthMessage`/`authenticate`/`createPassword`/`revokePersistentToken`/`authStatusStore`). The legacy `websocket-store.svelte.ts` wrapper (528 LOC) is FULLY DELETED — do not re-add it or a second `onMessage`/auth-mutation owner. A CI grep gate (`src/tests/deprecated-ws-store-gate.test.ts`) fails the build if the literal module name reappears anywhere in `apps/frontend/src`. `offline-state.svelte.ts`/`pwa-status.svelte` are the one deliberate exception — they read connection state straight off `$lib/rpc/client` (not `subscriptions.svelte`) to stay pre-auth-pure.
