...
│   │   │                      #   GoLiveCard.svelte (readiness + config rows + start, one adaptive surface)
│   │   │                      #   IdleCockpit.svelte (GoLiveCard → Preview disclosure → SourceSection → Roadmap disclosure)
...
│   │   │                      #     UNMOUNTED migration shims, kept-not-deleted (`TD-unmounted-source-shims`)
...
│   │   │                      #     config.source write itself, no longer purely presentational),
...
- Device-first source model + Go-Live cockpit [EXISTS]: `main/live/GoLiveCard.svelte` is the one adaptive readiness + config + start surface, mounted at the top of `IdleCockpit.svelte`. It derives Start-gating from the pure `lib/streaming/go-live-readiness.ts` (`deriveGoLiveReadiness`) against four gates (source/network/destination/engine) and collapses to a thin ready-bar once every gate is green and no config row is dirty. `LiveView.svelte` switches between `IdleCockpit` (pre-stream: GoLiveCard → Preview disclosure → `SourceSection` → Roadmap disclosure) and `LiveCockpit` (streaming: telemetry strip → bitrate adjuster → `IngestStats` → Stop) on the OPTIMISTIC streaming edge (`isStreaming || streamingOptimismState === 'starting'`) — never the raw `is_streaming` flag alone, so Start never flickers back to idle mid-launch. `SourceSection.svelte` renders the single `getSources()` list — fed by the backend's unified `sources` broadcast (folds pipelines/devices/device_modes into one ordered `StreamSource[]`) — as ONE `<ul>` (every `StreamSource` origin as a row, capture rows refilled in preference order, inline reorder on ≥2 capture devices) and owns the `config.source` write itself (`rpc.streaming.setConfig({source})`). `StreamSettingsCard.svelte`/`OnboardingChecklist.svelte`/`ServerReadiness.svelte`/`NetworkIngestSection.svelte` are no longer mounted anywhere — GoLiveCard/IdleCockpit absorbed every responsibility they used to own; the files are kept-not-deleted as a one-release rollback safety net (`TD-unmounted-source-shims` in `docs/TECHNICAL_DEBT.md`) — only `StreamSettingsCard`'s `ConfigRow` type is still imported (by GoLiveCard/IdleCockpit). See root `AGENTS.md` → "DEVICE-FIRST SOURCE MODEL + GO LIVE CARD" for the full backend+frontend contract.
