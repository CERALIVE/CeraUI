...
(CalVer, `2026.9.3` at time of writing) — the single source of truth, matching the platform's
...
- **REMEMBER-ME STORES A TOKEN, NEVER THE PASSWORD [EXISTS]:** `localStorage.auth` holds the device-issued, individually revocable `auth_token` from a persistent login. `authenticateWithToken()` and reconnect use `auth.login`'s `input.token` branch, never `input.password`. The auth store persists the issued token before publishing successful auth; an unchecked successful login removes a previous credential, and successful token reauthentication without a replacement retains the existing one. `SystemHelper.savePassword` delegates to `createPassword()`, which clears the remembered credential only after the device confirms the password change that revokes all outstanding tokens. `Layout.svelte`'s explicit clear-saved-session action also best-effort calls `revokePersistentToken`, without delaying the local clear. Extend `src/tests/auth-no-plaintext-password.test.ts` for any new credential writer. Device half: [`../backend/AGENTS.md`](../backend/AGENTS.md) → A REMEMBER-ME CREDENTIAL IS A REVOCABLE TOKEN; policy: root charter **G3**.
  **THE TAXONOMY IS THE CONTRACT, and it is board-proven.** `rejected` is the ONLY verdict that may delete `localStorage.auth`; `unreachable` — socket-not-ready, RPC error, timeout — must PRESERVE it, because a device that cannot be reached has said nothing about whether the credential is still valid, and deleting it there is how a backend restart turns into a password prompt the operator did not earn. Drill C2 exercised exactly that on a Rock 5B+: a remembered login survived a reload, then a REAL `systemctl restart ceralive` with two reloads inside the outage, `localStorage.auth` unchanged throughout, no password screen, and the session re-authenticated 1972 ms after HTTP returned; the unchecked-remember control ran in the same session and correctly landed on the password screen with the key absent. One browser-observation trap worth knowing before writing a similar test: a reload while HTTP is down moves Chromium to an error document, and CDP `DOMStorage.getDOMStorageItems` then fails `Frame not found for the given storage id` even though origin storage survives — assert through an in-memory `BrowserContext.storageState()` across that window and never persist that credential-bearing state to disk.
  **Residual, recorded by name: the token is device-issued and device-scoped only.** A CLOUD-issued server token that would let one operator credential span devices is a separate design and does not exist; it is registered as an engineering follow-up in [`../../docs/DESIGN-FOLLOWUPS.md`](../../docs/DESIGN-FOLLOWUPS.md) rather than implied here.
- **Store ownership (single owner rule) [EXISTS]:** `lib/rpc/subscriptions.svelte.ts` is the SOLE `rpcClient.onMessage` consumer and owns every non-auth reactive/connection getter (`getConfig`, `getStatus`, `getIsConnected`, `getConnectionState`, …). `lib/stores/auth-status.svelte.ts` is the SOLE auth-mutation and remember-me persistence path (`ingestAuth`/`getAuthMessage`/`authenticate`/`authenticateWithToken`/`createPassword`/`revokePersistentToken`/`authStatusStore`). Both login entrypoints return `AuthAttempt` (`ok | rejected | unreachable`); automatic credential invalidation requires server rejection, while socket/RPC/timeout failures preserve it for Retry or explicit clearing. On success persistence settles synchronously before auth state flips, so `Auth.svelte` must not recreate a persistence `$effect` that can lose the write on unmount. The legacy `websocket-store.svelte.ts` wrapper (528 LOC) is FULLY DELETED — do not re-add it or a second `onMessage`/auth-mutation owner. A CI grep gate (`src/tests/deprecated-ws-store-gate.test.ts`) fails the build if the literal module name reappears anywhere in `apps/frontend/src`. `offline-state.svelte.ts`/`pwa-status.svelte` are the one deliberate exception — they read connection state straight off `$lib/rpc/client` (not `subscriptions.svelte`) to stay pre-auth-pure.
- UI primitives: extend via shadcn-svelte CLI (`bunx shadcn-svelte@latest add <component>`), not by hand.
- Zero-warning gates: `bun run check` includes `svelte-check --fail-on-warnings`. `bun run build` delegates to `scripts/ci/build-warnings-gate.mjs`, which streams Vite stdout/stderr and fails on warning text, `INEFFECTIVE_DYNAMIC_IMPORT`, or the large-chunk notice. The script's `--scan-file` mode is the falsifiability seam; CI keeps calling the existing package scripts unchanged. **The gate started at 5 warnings in 4 files and two of them were `slider.svelte`, so the shadcn decision is written down here rather than left to the next reader.** That file is CLI-managed (`lib/components/ui/`), and the house rule above says primitives are extended through the CLI and never hand-edited — so it was regenerated to the upstream CLI output rather than patched warning-by-warning. What that regeneration DROPPED was a local hand-edit forwarding `aria-label`/`aria-labelledby` from the Root onto `SliderPrimitive.Thumb`; bits-ui puts `role="slider"` on the thumb, so a thumb with no accessible name is what axe's `aria-input-field-name` flags. It is a follow-up, NOT a silent acceptance: `aria-input-field-name` is absent from `tests/e2e/a11y-baseline.json` (which carries `color-contrast` only), so the gate would redden the moment a scanned destination renders a slider — the `EncoderDialog` bitrate slider is behind a dialog the destination-scoped axe run does not open, which is why nothing is red today. Do not re-apply the forwarding as a hand-edit to the CLI file; either pass the name at the call site or take the fix through the CLI.
...
- Destination deep-links dynamically import `lib/stores/navigation-lazy.ts`, the cycle-safe boundary that dispatches by the `live | network | settings` destination key. Callers must not dynamically import the already-static navigation/config modules directly: Rolldown correctly reports that as `INEFFECTIVE_DYNAMIC_IMPORT`. They also must not import that full graph statically from `LiveView`, which recreates its initialization cycle. The wrapper itself has no static importer, so its boundary remains effective.
...
- Effect lifecycle hygiene (experience-stability todo 22) [EXISTS]: dialog-open reads are generation-fenced, so a promise from a closed or superseded open cannot overwrite the current dialog; `HotspotDialog` seeds its draft on the open edge and abort-fences QR generation. Document listeners return their exact removal callback. Repeating async recovery work uses a re-armed `setTimeout` only after the prior read settles, never `setInterval`. Derived store projections own stable list/object snapshots instead of allocating from public getters. Render lists key on semantic identity: channel lane, pulse lane, trace lane/segment/gap timestamp identity — never a loop index.
...
**`PLATFORM_CAPS_BY_HARDWARE` (`ValidationAdapter.ts`) STAYS and is NOT the same class of thing.** It is a platform ENCODE CEILING — what the BOARD's encoder can emit — never device capability. Live `get-capabilities.platform` and `encoders[]` now outrank it; the map is fallback-only for a legacy/absent encoder ladder and is pinned to the engine platform golden by `ValidationAdapter.caps.test.ts`. `intersectCaps` takes the INTERSECTION of it with the device's own ladder, so it can only ever SUBTRACT from the offering and can never add a mode the device did not report. Do NOT delete this map: removing the fallback ceiling would let a legacy session offer encode targets the SoC cannot produce.
...
- Start watchdog + attempt generations + typed failure rendering (Todo 29) [EXISTS]: the optimism store carries a monotonic `generation` (bumped on every `transitionToStarting`) and a typed `failure` (`StartFailure`). A revert (`revertWithReason`/`revertWithFailure`) tagged with an OLDER generation is a stale response from a superseded attempt and is IGNORED — `LiveView.handleStart` captures `getStreamingAttemptGeneration()` right after `startStreamingOptimism()` and passes it back on revert, so a delayed reply from an older start attempt cannot clobber a newer one. The authoritative-status precedence rule is UNCHANGED (a contradicting `is_streaming=false` push during `starting` is still ignored — no flicker); generation fencing applies ONLY to the explicit revert path. `lib/rpc/streaming-start-watchdog.ts` is the sibling of the stop watchdog (`streaming-stop-watchdog.ts`): armed on `transitionToStarting`, it fires at `START_WATCHDOG_TIMEOUT_MS` (90s, above the bounded-retry budget), PULLS authoritative status via `rpc.status.getStatus`, and reconciles — streaming ⇒ confirm `starting`→`idle` (lost success push), NOT streaming ⇒ revert with `start_timeout` (lost failure reply / wedged attempt). This is the "no infinite spinner" guarantee. `LiveView` renders the typed failure: the `failed` RPC result echoes `StartFailure` (phase+class+retriable+attemptId, on `streamingStartOutputSchemaExtended`); `startFailureMessage(failure)` maps `class` → `live.startFailure.class.*` + a retry-state suffix (`retriedThenFailed` retriable / `notRetriable` deterministic), falling back to the legacy `live.startFailed.*` code path. **One class is keyed by CAUSE.** `capture_source_unavailable` arrives with the additive optional `captureCause` (`negotiation_failed` / `no_signal` / `device_busy` / `composition-unsupported` / `secondary-unavailable`, the closed `START_FAILURE_CAPTURE_CAUSES` enum in `@ceraui/rpc`), and when the failure carries one the toast key is `live.startFailure.class.<class>.<captureCause>`; the class-level key has NO string of its own for this class (the test helper re-nests the flat catalog, so a key that is both a string and an object would collide), and a cause-less instance falls through to `live.startFailed.generic`, which the backend mapper never produces. The class alone names no action (an unsupported signal format, a dead cable and a momentarily-held input want three different responses), so a surface that renders it MUST read the cause. `retriable` is already materialized by the backend per cause (`device_busy` is the only retriable one, at `start-rpc`), so the suffix rule above is unchanged. `LiveView` also passes `failure.captureCause` to `IdleCockpit` → `CompositionCard` as `startFailureCaptureCause`; that card renders an inline refusal (`live.startFailure.class.capture_source_unavailable.<cause>`) for the two composition causes ONLY, because it owns the second leg. Every other cause belongs to the source card, which owns the primary leg, and gets the toast alone. Backend contract: `apps/backend/AGENTS.md` → START-FAILURE DIAGNOSTICS → "…AND A CAPTURE REFUSAL CARRIES ITS OWN CAUSE". It deliberately does NOT concatenate the engine's raw `failure.message` — doing so shipped `invalid params: audio-device-unavailable: ALSA capture device 'hw:CARD=rockchiphdmiin' is busy or unavailable` verbatim to an operator with no console (found in Wave H device QA). The diagnostic still reaches the backend logger and so the Settings → System Logs download; the toast stays plain-language, and no user-facing string names `journalctl`, a systemd unit, or a shell command. Gate: `src/tests/operator-copy-no-internals.test.ts` sweeps all 10 locales. **RPC-timeout contract:** `client.ts` `PROCEDURE_TIMEOUT_MS` raises `streaming.start` to 75s (above the 60s retry budget, below the 90s watchdog) — the 30s default would reject a still-valid in-flight retrying start and drop the terminal typed `failure`. Coverage: `streaming-optimism.test.ts` (generation + typed revert), `streaming-start-watchdog.test.ts`, `streaming-status-broadcast.integration.test.ts` (authoritative false frame reaches+consumed through the WS layer), `client-timeout.test.ts`, `live-start-failed-reason.test.ts` (typed rendering, incl. the per-cause `capture_source_unavailable` toast for the three capture causes and the copy sweep proving it names the signal format and the camera-side fix, never the operator's settings). The two composition causes have no rendered-DOM test on `CompositionCard` yet; that is a known gap, not a claim. Nothing here is board evidence: the full board drill with released artifacts is hardware-gated and has not run.
  **A closed/replaced socket ends every request immediately.** `client.ts` rejects and clears every pending call in the same tick with the exported `ConnectionResetError`, carrying the procedure path; callers no longer wait for the unchanged 30 s default or 75 s `streaming.start` timeout. `LiveView.handleStart` deliberately preserves `starting` for this one transport error because the start may already have reached the backend; the watchdog's authoritative pull decides success versus timeout. `osCommand` classifies it as `connection_reset` and uses its existing transport-failure feedback. Ordinary dialog catches keep their existing toast/inline behavior. Heartbeat staleness alone rejects nothing: only the resulting real socket close crosses this boundary.
...
- …AND A TRANSIENT GAP IS HELD FOR ONE DISPLAY WINDOW [EXISTS]: `METER_UNAVAILABLE_DISPLAY_GRACE_MS` (1 200 ms, same pure module) is how long a gap BETWEEN TWO LIVE READINGS is held before it may draw the `unavailable` band; `LiveAudioMeter` holds the reading already on screen for that long and publishes `data-gap-held`. This is a DISPLAY rule and nothing else — the band is honest and it is also instantaneous, so one dropped frame between two healthy readings drew the full "Meter unavailable" treatment for a single paint and took it away again, which on a board reads as the meter blinking, and an operator cannot tell a blink apart from the real thing. Six properties are load-bearing. It holds the reading ALREADY ON SCREEN rather than blanking the bars (blanking is a different flash, not the absence of one), and a gap with nothing behind it — the meter's first ever frame, or one arriving onto an already-dead meter — bands immediately because it has nothing to suppress. It arms ONE deadline per gap and never re-arms it from the repeat frames inside that gap, so a feed that keeps publishing `unavailable` at the engine cadence still bands at the deadline instead of debouncing forever. Recovery is NOT graced: the next live frame drops the gap and the real reading is back in the same paint. `pending`, `stale` and `superseded` are deliberately EXEMPT — each answers a question this window has no evidence about, and delaying any of them would hide a real outage rather than a paint. So are the two STATED reasons, `mode_none` and `embedded_audio` (`STATED_UNAVAILABLE_REASONS`, read by `isTransientMeterGap`): both are answers to something somebody just did — the operator's own "No audio" pick, and a property of the selected source — and both are published SYNCHRONOUSLY by the backend's selection-change broadcast (`audio-meter-bridge.ts` `noteMeterSelection`), i.e. squarely inside this window, so delaying either would make a deliberate action look laggy. A gap carrying NO reason at all is treated as transient — an unnamed gap is the weakest claim on the wire, and it is the one worth not flashing. The window is sized between the two clocks either side of it: ~6× the engine sidecar's ~200 ms cadence, and strictly BELOW `AUDIO_METER_STALE_MS` (2 s) or the staleness watchdog would draw its own band first and the grace would be unreachable. Do NOT raise it past that deadline, do NOT extend it to the three exempt states, and do NOT "fix" a future blink by debouncing the backend's gaps instead — that changes what the device SAYS, not what it paints. Backend half: `apps/backend/AGENTS.md` → the display-grace note under IDLE AUDIO-METER DEVICE PREFERENCE. Coverage: `audio-meter-liveness.test.ts` (`isTransientMeterGap` + the exempt-reason table) + `LiveAudioMeter.grace.test.ts` (rendered DOM under fake timers — the held single-frame gap, the band at the deadline, the un-re-armed repeat frames, ungraced recovery, and each exempt state).
- A reading NAMES the device it is a reading OF [EXISTS]: the meter drew bars and nothing else, so on a board with several cards an operator could not tell which one they were looking at — and that is worst exactly where it matters most, because when the engine's own pick disagrees with theirs the bars are the ONLY place that disagreement is visible (the picker above happily reports the card they chose). `meteredAudioLabel` (`lib/streaming/sourceSummary.ts`) resolves `level.source.identity` against `status.audio_sources` on `stable_id` — which is precisely what the engine puts in that field — and `LiveAudioMeter` renders it as `data-testid="live-audio-meter-device"`. Two rules are load-bearing. An identity the device publishes NO source row for yields NOTHING, never the raw id: `?? identity` would print `card:USB Audio` at an operator. And the label is gated on the SAME `dead` predicate the bars are, so a pending, stale, superseded or engine-`unavailable` reading names no device — labelling a retired reading is the "bars under the wrong name" defect the selection gate below exists to prevent, one step quieter. Because of this the meter now reads THREE subscription getters (`getAudioLevel`, `getConfig`, `getStatus`) and a test that mounts it — directly or via `SourceSection`/`LiveSummaryStrip` — must mock all three. Coverage: `LiveAudioMeter.device.test.ts`.
- A source that brings its OWN audio has no device to pick [EXISTS]: an RTMP/SRT row carries its audio muxed into the ingested stream ("Includes audio"), and selecting one used to write `config.source` and nothing else — so a device-based `config.asrc` chosen for the previous camera stayed on disk and on screen, the picker went on offering "DJI Mic Mini" for a stream whose audio comes from no card at all, and that stale pick is what a later start would resolve. `lib/streaming/embedded-audio-selection.ts` is the pure rule: `planAudioSelectionForSource` clears a DEVICE pick onto `Auto` when the target `carriesEmbeddedAudio`, and does nothing otherwise. Four properties are load-bearing. It keys on the declared `audioKind === 'embedded'`, never on `origin === 'network'` — a network source whose engine reports `selectable` still wants a device. It never overwrites a pick the OPERATOR made themselves (an explicit "No audio" on the ingest row survives). The `asrc` travels WITH the `source` write in ONE `setConfig`, so a start can never resolve a pick the switch was supposed to retire. And **switching back restores NOTHING** — an earlier revision remembered the displaced pick in session-module state and re-applied it on return, which is the very "remembers the last device audio selection" the report asked to remove, and which a board proved never fired; `Auto` is the honest landing state because it re-resolves to the returned-to source's own device. The plan function takes no memory argument, so the behaviour is unexpressible rather than merely removed. Coverage: `embedded-audio-selection.test.ts` + `SourceSection.test.ts` → "a source that brings its own audio has no device to pick".
- …AND IT HAS NO DEVICE TO PICK, IN ANY CONNECTION STATE [EXISTS]: the rule above retires a STALE pick on the switch; this is the surface itself. `audioEmbeddedActive` used to read `audioKind === 'embedded' && capabilities?.network_embedded_audio === true`, so an ingest row fell back to the full ALSA picker whenever that capability was not positively `true` — and it is ABSENT from the engine-starting and engine-unavailable fallback snapshots, so every engine blip handed the operator a list of unrelated microphones for a source whose audio comes from no card at all. It now reads `activeSource?.audioKind === 'embedded'` alone, in `SourceSection.svelte` AND `AudioDialog.svelte` (kept in lockstep — the dialog offering a device the picker says does not exist is the same lie twice). Two facts are deliberately kept APART: whether the SOURCE owns a card (this one, always false for rtmp/srt) and whether the ENGINE can ROUTE the muxed track (`network_embedded_audio`, which keeps its own surface as `IdleCockpit`'s roadmap ComingSoon pill). Do NOT re-add the capability to this gate. Coverage: `SourceSection.test.ts` → the embedded describe (no-capability, waiting-for-a-publisher across all three capability shapes, and SRT) + the M2 matrix row, which previously pinned the defect. Backend half: `apps/backend/AGENTS.md` → “…AND A SOURCE THAT BRINGS ITS OWN AUDIO HAS NO CARD TO METER”.
- …AND ITS METER SAYS SO RATHER THAN SUBSTITUTING ANOTHER CARD [EXISTS]: with no publisher connected there is nothing to meter, and the backend now says exactly that — `{unavailable: true, reason: 'embedded_audio'}` instead of the idle sidecar's unrelated card. `AudioLevelMeter` resolves it through the existing `live.preview.audioUnavailableReason.*` lookup (no component change), and `LiveAudioMeter`'s `dead` predicate already gates the device line, so a retired reading names no device. Once a publisher connects the engine meters `MeterMode::ProgramAudio` off the program leg and tags the level `streaming`; the backend forwards that verbatim, so the bars an operator sees mid-stream ARE the embedded track. Coverage: `LiveAudioMeter.test.ts` → "an ingest source's audio arrives with its stream".
- …AND A SECOND WRITER OF `asrc` MUST TELL ITS OWNER [EXISTS]: `LiveView` keeps a write-through shadow of the audio pick (`audioOverride`), and `effectiveAudioSource` prefers it over `config.asrc` — so the picker renders the shadow, not the device's value. `handleSelectAudioSource` was the only writer, so the shadow could never drift. The source switch above made `SourceSection` a SECOND writer, and the shadow went stale: board-measured, the picker read "DJI MIC MINI" for 16 s while the device held `Auto` and the meter truthfully named "HDMI Input" — the picker contradicting the reading beside it, which is the same class of lie the meter's own device line exists to prevent. `SourceSection` therefore takes `onAudioSelectionApplied` and calls it with the written value on the APPLIED echo only (never on a refusal, and never when the plan left `asrc` alone), and `LiveView` folds that into `audioOverride`. Do NOT add a third `asrc` writer without routing it through the same callback, and do NOT "simplify" the picker to read `config.asrc` directly — the shadow is what keeps codec/delay together with the pick. Coverage: `SourceSection.test.ts` → the four `onAudioSelectionApplied` cases (both silence directions included).
- A level belongs to the PICK that produced it [EXISTS]: `trackMeterSelection`/`isLevelSuperseded` (same pure module) retire the on-screen reading the instant `config.asrc` changes, so the previous device's bars never render under the new selection's label. Wave H board QA: with "Audio source: No audio" selected the meter drew active green bars for several seconds — the backend half is `apps/backend/AGENTS.md` → "No audio is NOT Auto", and this is the frontend half, which must not keep drawing while the backend's confirming broadcast is in flight. It holds ONE frame by REFERENCE (every broadcast is a fresh object, so identity is an exact "this is the frame I already had") and releases on the very next frame however fast it arrives — a one-frame gate, never a mute, because the meter is a preference, not a pin. `observed` distinguishes "no pick seen yet" from a genuinely unset `asrc`, without which the first render would supersede its own first frame. It also drops the stale frame's `reason`, so a retired `not_selected_device` is never relabelled onto the new pick. **`config.asrc` is not the whole pick, though — under `"Auto"` it is a SENTINEL, and the card behind it moves with the selected VIDEO source (and with the engine's audio list).** So this gate cannot see an Auto re-resolution at all: `asrc` is `"Auto"` before and after. Do NOT try to fix that here by keying the gate on `config.source` as well — the frontend cannot know which of the six Auto rules fired. The backend already retires it at the source of truth, by broadcasting an explicit `handoff` gap whenever the resolved `(silenced, preference)` pair changes (`apps/backend/AGENTS.md` → IDLE AUDIO-METER DEVICE PREFERENCE), which arrives here as an ordinary `unavailable` frame and needs no special handling — it is an unstated reason, so the display grace above holds it like any other transient gap, and a handoff the engine completes inside the window is invisible rather than a blink. `LiveAudioMeter` therefore reads `getConfig()` as well as `getAudioLevel()` — a test that mounts it (directly, or via `SourceSection`/`LiveSummaryStrip`) must mock BOTH. Do NOT "fix" a future instance of this by widening the staleness watchdog: that watchdog is precisely why the bug read as "a few seconds of bars, then it settles", and a regression test that waits it out passes on the broken tree. Coverage: `audio-meter-liveness.test.ts` (8 pure cases) + `LiveAudioMeter.selection.test.ts` (rendered DOM, asserting INSIDE the staleness deadline).
...
- AP+STA concurrent rows [PARTIAL — implemented; hardware validation pending]: `supports_ap_sta_concurrency === true` is the only admission signal. A capable station row keeps Connect and bond controls while its hotspot is active, exposes per-radio setup/stop controls, and starts without the destructive exclusive-mode confirmation. A missing/false flag retains the existing one-mode-at-a-time confirmation and the dialog states that WiFi will disconnect. `isApRadio()` treats an explicit `mode: "station"` as authoritative even when a concurrent `hotspot` block is present; `NetworkView` separately uses `hotspotIsActive()` to populate the Hotspot section. Full contract: [`../../docs/AP-STA-CONCURRENT-MODE.md`](../../docs/AP-STA-CONCURRENT-MODE.md).
- **Bonded modem signal glyph parity [EXISTS]**: `buildBond()` carries a router-mode dongle's normalized `router_admin.signal` tier when no ModemManager percentage exists, using the same `resolveSignalInstrument` precedence as the Cellular card. `LinkIndicator` renders that tier through the same three-bar cluster as a percentage, while preserving the `no_sim`, unknown-reading, and managed-modem fallbacks; no percentage is fabricated. `BondedLinksSection` names tier-only readings with the shared signal vocabulary, and `LinkBadge` is the shared row/HUD composition so both surfaces use one glyph. Coverage: `signal.test.ts`, `hud.test.ts`, `LinkBadge.test.ts`, `BondedLinksSection.signalParity.test.ts`, and `tests/e2e/visual/bonded-link-signal.visual.spec.ts`.
- **Tier-only bonded readings stay capability-honest [EXISTS]**: a device-admin tier is used only when the percentage is absent and the reading is known; `device-stack` and empty-slot readings never resurrect a suppressed modem signal. Numeric percentages remain limited to measured values, compact HUD badges remain ordinal-plus-glyph only, and the Cellular card keeps ownership of router provenance and freshness presentation.
- **Bonded Links is THE BOND, and presence in it is BINARY [EXISTS]**: `isBondMember(netifEntry)` (`lib/stores/hud/link-status.ts`) mirrors the backend's eligible bond pool, used by `genSrtlaBondEntries()` / `genSrtlaIpList()`. An included link has exactly one row; an excluded link has none, never a dimmed ghost or an `Excluded` badge. Membership requires `enabled:true`, an address, and no blocking error. **The exact `NETIF_DUPLICATE_IPV4_ERROR` string is allowed only alongside `enabled:true`**, because `netIfBuildMsg().enabled` IS `isBondCandidate()`'s verdict, including mappability, other error bits and operator opt-out. The string alone is lossy: a compound error can serialize identically while `enabled:false` preserves the refusal. Every other nonempty error remains excluded. Eligibility exists while idle; `linkTelemetry` and `bond_mapping` describe session realization and must not become prerequisites for listing eligible links. `buildBond()` returns `{links, unbondedCount}`; `HudState.unbondedLinkCount` feeds `bonded-links-not-bonded`, while per-device rows own explanations. The remaining-interface sweep is membership-based, never `eth*`/`enx*`-based (router dongles and old-image `dg<N>h` veths must not vanish by name); modem/wifi ids are claimed before the sweep to prevent duplicates; loopback is skipped by address through `isLoopbackIpv4`. `LinkSignal.bondExcluded` remains deleted. HUD and Network share this single derivation. Coverage: `hud.test.ts`, `hud/bond-membership.test.ts`, `BondedLinksSection.test.ts` and `tests/e2e/twin-bond-membership.spec.ts` (captured twin wire entries through real ingestion).
...
- Bluetooth card [EXISTS]: `main/network/BluetoothSection.svelte` renders the BlueZ surface on the Network destination, driven by the `bluetooth` broadcast (`subscriptions.svelte.ts` → `getBluetooth()`). Its derivations are the pure, rune-free `main/network/bluetooth-view.ts`. Five rules are load-bearing. **(1) The master enable toggle is PESSIMISTIC** — `osCommand` WITHOUT `confirmOnResolve`, the `NetworkIngestDialog` switch verbatim in discipline: the position moves only when the confirming `bluetooth` broadcast lands, and the spinner is the SOLE optimistic element. **(2) "The operator switched it off" OUTRANKS the stack's own cause.** `BluetoothStack` records an operator-disabled device as `bt_unavailable{bluez_unavailable}` — correct from its point of view (it is not observing BlueZ) and the opposite fact to an operator — so `bluetoothSurface` applies the SAME gate order `bluetooth.procedure.ts` applies to a mutation, with the mock provider's one documented divergence: `emulated` is answered FIRST, because telling someone to switch Bluetooth on when the host has no radio is advice they cannot act on. Rendering the cause literally would blame a healthy service for a setting. **(3) EVERY typed refusal renders ON SCREEN, beside the control that was refused** — `classify` keeps a structured `{success:false}` `ok` so it never takes `osCommand`'s toast path, and only a THROWN rpc (a transport fault with no typed reason) keeps it. The kiosk touchscreen cannot hover, and the shared enum's fourteen members each name a different operator action; `unit_missing` remains distinct from a service that is installed but failed to start. `bluetoothRefusalKey` is a TOTAL record over the enum, so a fifteenth refusal fails the build rather than reaching an operator as its own dotted path. **(4) Absence renders as absence** — `battery` is omitted when the device exposes no `Battery1` (BlueZ retracts the whole interface on disconnect, so a retained 80 % would assert a level nothing measured), and a board with no adapter gets NO scan control rather than a disabled one, because there is no capability being withheld. **(5) The `audio-input` hint is gated on CONNECTED and the active backend** — a bonded-but-disconnected mic has no PCM behind it, and a connected PipeWire microphone cannot be offered while ALSA is selected. The latter renders an inline instruction to choose PipeWire instead of a misleading Live-source pointer. The pairing-agent gap (`agent.reason === 'exporter_unavailable'`, true on every real board today) is stated up front while an unpaired device is on screen, because the wire carries it precisely so a surface can say so before the operator taps. Copy: `network.bluetooth.*` (50 keys × 10 locales). Coverage: `bluetooth-view.test.ts` (the gate-order table, the 14-refusal completeness sweep across all ten catalogs plus its falsifiability control), `BluetoothSection.test.ts` (empty / scanning / `bt-mic-paired` / backend refusal / every mutation refusal path, incl. the RPC-success-alone-does-not-move-the-switch proof), `tests/e2e/bluetooth.spec.ts` and `tests/e2e/visual/bluetooth.visual.spec.ts`.
...
- The wired port ROLE is STAGED, and Save is its ONLY dispatch site [EXISTS]: `main/network/EthernetRoleSelector.svelte` renders the Uplink / Shared LAN rungs and dispatches NOTHING — it owns no RPC and no async-op. The rung the operator picks is dialog state held by `main/dialogs/NetifDialog.svelte` (`stagedRole`, alongside `enabled`), written back through the control's `onSelect`, and `NetifDialog.save()` is the only caller of `rpc.network.setEthernetRole` anywhere in the app. It used to apply on the radio click itself, which made a change that reconfigures the port — and can drop the very LAN path the page is being read over — reachable by one stray tap on the shipped kiosk touchscreen, with no way back. Five rules are load-bearing. **(1) A staged change is VISIBLE**: a picked rung is indistinguishable from the rung the device is already on, so a staged role that DIFFERS from the applied one renders a standing band (`eth-role-staged-<name>`, from the pure `deriveEthernetRoleStagedWarning`) saying the change applies on save and can drop LAN connectivity to this device — it STANDS rather than toasts, because a kiosk touchscreen cannot hover and an expiring notice is how a pending change comes to read as an applied one. **(2) It warns in BOTH directions** — leaving `shared-lan` reconfigures the port just as much as entering it — and is SILENT when the staged role equals the applied one. **(3) The streaming interlock ESCALATES that band rather than adding a step**: a port that is CURRENTLY a bonded member, while a stream is live, appends `deriveEthernetRoleConsequence`'s own sentence (`eth-role-staged-consequence-<name>`) beside the reachability one, never instead of it. The former arm/confirm two-step is GONE — Save is the confirmation gate, and a confirm on top of a confirm is one operators learn to click through. **(4) The role leads the save**: it dispatches BEFORE `network.configure`, and a refused role change stops the save outright, because a shared-LAN port is excluded from the bond by the device so the toggle beneath it means nothing until the role has settled. A staged role EQUAL to the applied one dispatches nothing at all, so an untouched save is byte-identical to the pre-staging one. **(5) The BACKEND is untouched** — same procedure, same keyed op (`ethernetRoleOpKey`), same `silent: true` inline refusal, same device-moves-the-control settlement on the terminal `eth_role` frame (the op stays `pending` past the RPC and the displayed role holds on the PRIOR one). Cancel discards the staged role and the open edge resets it. Copy: `network.ethRole.staged.{title,body}` (10 locales); `network.ethRole.description` no longer claims the change "takes effect right away". Coverage: `ethernet-role-view.test.ts` (the pure warning, incl. an exhaustive proof that it escalates on exactly the interlock's own verdict), `NetifDialog.role.test.ts` (staged-not-applied, exactly-one dispatch on Save, the band's render/silence table, Cancel discards), and the S7 guard `tests/device-actions-async-op.guard.test.ts`, whose `eth-role` entry now points at the dialog.
- EVERY cellular device gets a row, and an uncontrollable one is DIMMED-WITH-A-REASON, never hidden (modem-stack Phase B, todo 26) [PARTIAL — never rendered against real cellular hardware]: `main/network/CellularSection.svelte` renders ONE calm summary row per device off the Wave-4 additive wire fields (`device_class`, `availability_reason`, `slot_label`), and `main/network/cellular-row.ts` is the pure, rune-free derivation behind it. Three classes share the row: `mm-managed` (`usb`/`pcie-mhi`/`pcie-mtk`/`soc-qrtr`, and an ABSENT `device_class` — the pre-Phase-B wire came from mmcli only, which lists nothing else), `router-ethernet`, and `unmanaged` for a transport this build does not recognise. The BAND vocabulary is `docs/MODEM-SUPPORT-MATRIX.md` §1's verbatim, but the BADGE's is not, and that split is the §3 OL-1 fix: the copy used to be the band with a capital glued on (`MM-managed`, `Router-ethernet`), which is the name of the daemon that controls the device rather than anything an operator can act on. It now reads `Directly managed` / `Router dongle` / `Unrecognised`; the band itself stays on `data-class-band` and in the diagnostics `transport` row. `CellularSection.test.ts` asserts BOTH directions — the three operator words, and that no badge's text contains its own band. Six rules are load-bearing:
  ...
  A udev-only row begins as `modem_initializing` and becomes `undriveable` only
  after two authoritative roster cycles still cannot claim it. The row remains
  present and non-actionable until physical detach; `cellular-row.ts` resolves
  both the state and reason through keyed copy, gives the state the calm attention
  treatment, and makes its one undriveable reason supersede the generic no-address
  note so persistent hardware does not become a wall of duplicate warnings.
  Copy: `network.cellular.*` (20 keys × 10 locales). Coverage: `cellular-row.test.ts` (the pure tables) + `CellularSection.test.ts` (the rendered state table — mm-managed healthy/registering/locked, router up/acquiring/down, unrecognised transport — plus the 44px touch-target, never-a-dotted-key and never-a-raw-token locks). **The `BondToggle` switch reaches its touch target through its HIT AREA, not its box** (closed app-wide in `DESIGN.md` §9 pass 3): `app.css`'s `[data-layout-mode='touch']` lift grows `[data-slot='switch']::after` to `--touch-target-min` instead of adding the switch to its `min-height` list. Growing the box would stretch the painted track into a 44px pill and strand the thumb, whose checked position is a translate off its own width. That override is deliberately UNLAYERED — the inset it beats is Tailwind's `after:-inset-y-2` in `@layer utilities`, and layer order outranks specificity.
- …and the row now answers FOUR questions inline, not ten (todo 64) [EXISTS]: ten todos each deposited one fact on that row and nobody ever removed one. Measured on the bench board with its real seven-device roster, the Cellular section was **864 px** tall: an `MM-managed`/`Router-ethernet` class pill on all seven rows, a `·`-joined inventory strip carrying APN + firmware + IMEI + serial, and a 140-character sentence about a web interface the operator's browser is not on — repeated verbatim four times. The one line that mattered, "The carrier doesn't allow service in this area for this SIM.", was the shortest thing on screen. The row now splits: **PRIMARY** is WHICH device (`modem-name`, `modem-slot-badge`), WHAT it is doing (`modem-state-badge`, `modem-carrier-badge`, `modem-lock-badge`, `modem-roaming-badge`), HOW the radio is (`modem-signal`), and WHY it is not working (every `modem-note`); **SECONDARY** is one per-row disclosure (`modem-details-toggle` → `modem-details-body`) holding `modem-class-badge` + its now-VISIBLE hint, `modem-detail`, `router-admin-facts` and `router-admin-note`. Four rules are load-bearing. **(1) The reason lines DO NOT fold.** Every one of them is also a disabled control's reason, and rule 3 above requires those on screen because the shipped kiosk touchscreen cannot hover to reveal a tooltip — folding them would trade a real honesty invariant for pixels. **(2) The roaming badge DOES NOT fold**, but nothing else about roaming is inline: it renders only while the modem is ACTUALLY roaming, i.e. only while money is being spent, which is the definition of relevant. **(3) The body stays MOUNTED and goes `inert` while collapsed** — none of it is sensitive (unlike the SMS inbox, whose `{#if}` gate is a privacy property), so keeping it in the DOM makes it one paint away instead of one fetch away, and `inert` is what stops a keyboard operator tabbing into a zero-height panel they cannot see. Do NOT swap `inert` for `aria-hidden`: that hides the subtree from AT while leaving it focusable, which is the axe `aria-hidden-focus` violation rather than a fix for it. **(4) The reveal is the todo-39 CSS `grid-template-rows: 0fr → 1fr`, never a JS transition** — a JS transition compiles to the Web Animations API and escapes BOTH global motion freezes. **(5) THE PRIMARY REGION HAS A BADGE BUDGET, and it is FOUR** (todo 29). This split is not undone by one bad commit — it is undone by ten good ones, each adding the one pill its own feature obviously needed, which is exactly how the 864 px wall was built the first time. The ceiling counts `[data-status-badge],[data-no-sim]` OUTSIDE the disclosure body (a pill the operator asked to see is not part of the budget), and four is what the bench's roaming Quectel already reaches: state + carrier + roaming + slot. Copy: `network.cellular.details.toggle` (10 locales). Coverage: `CellularSection.density.test.ts` (the ancestry-based primary/secondary matrix, the reachability lock proving every moved reading is still rendered, per-row independence, distinct body ids, the 44 px touch target, and the badge budget with its own non-vacuity control). NOTE for tests: jsdom implements `inert` as a property but never reflects it to an attribute, so assert `el.inert`, never `hasAttribute("inert")`.
...
- A cpufreq policy is rendered as the thing it GOVERNS, never as a directory name [EXISTS]: `lib/system/cpu-freq.ts` (`deriveCpuFreqRows` / `cpuFreqBar`, pure/rune-free) folds `device-stats.cpuFreq` into rows for `DeviceStatsSection`. The panel used to print the collector's payload verbatim — `policy0` / `policy4` / `policy6` and no governor — which is honest and unreadable, and on a per-core x86 box produced a dozen near-identical bars. THREE shapes, published as `data-cpufreq-shape`, and the discriminator is STRUCTURAL rather than a board guess: **cluster** (a policy governs several CPUs, so it keeps its row and reads `Cortex-A55 ×4` with the governor as a chip and `policy0 · cpu0-3` beneath it), **per-core** (every policy governs exactly ONE CPU, so the policy is not a unit worth a row and identical ones fold into one row per `(label, ceiling)` group), and **raw** (the device sent no metadata — byte-identical to what shipped before, sysfs ids and nothing else, which is what an older backend still gets). Four rules are load-bearing. **NOTHING INVENTS A NAME**: a row is named by the device's own `label` or by the sysfs id, never by list position, core count or board model — and folding additionally REQUIRES a label on every policy, because a group has to be called something and the first policy's directory name is not a name for the eleven behind it (an unlabelled per-core board therefore renders twelve honest rows). **A FOLDED ROW REPORTS THE RANGE**, not one figure: twelve cores have no single current clock, so the reading is `0.80–0.81 GHz / 4.80 GHz` and the bar is a `span` rather than a `fill` — collapsing it would pick a core to be about. **A FOLDED ROW NAMES NO POLICY ID**: the id stays visible only where it names exactly one thing, so a group's detail line is the CPU range alone. **The ceiling is `maxKhz`** (`cpuinfo_max_freq`), so the bar does not wander when the governor moves, and a group is split per distinct ceiling so a hybrid board never draws one bar against a denominator half its cores never had. The governor chip renders the kernel token verbatim (its `title` explains it, `settings.deviceStats.cpuFreqGovernorHint`) — the fleet runs `performance` by design, so `cur == max` at idle is expected. Backend contract: `apps/backend/AGENTS.md` → CPU-FREQUENCY METADATA. Coverage: `lib/system/cpu-freq.test.ts` (pure — the three shapes, the no-label no-fold rule, the hybrid split, the disagreeing-governor withhold, and both bar geometries) + `main/settings/DeviceStatsSection.test.ts` (rendered DOM, incl. the raw byte-compat leg asserting no count, no chip and no detail line).
...
- Modem-surface a11y gate (modem-stack Phase B todo 29; extended to `DESIGN.md` Pass 3) [EXISTS]: `tests/e2e/modem-a11y.spec.ts` covers the cellular rows + the modem dialog with **seven** PASS/FAIL legs — scoped axe, a focus-trap + keyboard-only USB-confirm walk, an `ar`-locale RTL containment sweep asserted by `getBoundingClientRect()` (never a screenshot review), the same containment in the base locale at 375/768/1280 + the 1024x600 kiosk for rows AND dialog (BP-1…BP-3), the eight non-base catalogs at 375 (LO-1…LO-5), a `prefers-reduced-motion` leg (RM-1…RM-4), and a touch-target inventory (TT-1…TT-5). **The CI step's `a11y.spec.ts` filter is a path REGEX, so `modem-a11y.spec.ts` matches it as a substring and runs in that same job with no workflow change** — do not rename this file to something that stops matching.
  ...
  **(4) LO-4 asserts a state badge's WIDTH, not its text.** Tofu and a dropped glyph both collapse the box while the text still reads back, so a text assertion passes on a locale that renders nothing. One deviation is pinned as an EXACT SET so it reddens in BOTH directions (a fix must update the pin; a new regression grows the set): axe `color-contrast` on the semantic `Badge` palette — `text-status-<tone>` on a 10%-alpha `bg-status-<tone>` tint of the same hue trips the rule on EVERY status pill in the app (`netif-link-local`, the SourceSection pills and `AudioDialogContent` all predate wave 5 and use the identical pair), so this is the already-baselined app-wide token debt, not this effort's, and repairing it means re-toning `--status-*` app-wide; the gate additionally asserts every row's state badge carries a WORD and a GLYPH, so the palette can never be the sole carrier of a state. The touch-target set is now EMPTY, and its leg measures the HIT AREA rather than the box: it reads the `::after` overlay's own resolved insets, because this gate runs with the modem dialog open, where the scrim and the dialog's own scroll leave most controls answering no hit test at the moment of measurement.
...
- `@visual` **`DESIGN.md` pass-4 confirmation** [EXISTS]: `tests/e2e/visual/modem-pass4.visual.spec.ts` is the CONFIRMING round for the modem surfaces — it introduces no rule and changes nothing under `src/`. It walks fourteen state classes — the five section/dialog classes (`roster` · `no-sim` · `initializing` · `dialog-primary` · `dialog-advanced`), the three §1 capability renderings (`capability-absent` · `capability-unknown` · `capability-offered`), and the six dongle-credential classes (`lock-open` · `lock-locked` · `lock-auth-failed` · `lock-locked-out` · `lock-unlocked` · `lock-clear-armed`) — at 375/768/1280 in `en`, the five section/dialog classes again at those widths in `ar` (RTL) and `ja` (CJK), and all fourteen on the 1024×600 kiosk in touch layout — **86 PNGs** into the gitignored `test-results/design-pass4/34/` (Rule D). A fifth leg asserts the set is complete ON DISK, because the stop condition says the screenshots must EXIST; it and the kiosk leg walk ONE shared `KIOSK_SURFACES` constant, because two copies of that literal are one edit away from a capture the completeness check does not know to demand. Every capture is gated first by the shared containment probe, by IH-1 re-measured in pass 1's own selector vocabulary (`STATE_SIGNAL_ACTION` + `[data-hardware-tag]`), by row reachability, by dialog-box containment, and by a raw-token scan over operator text with the marked diagnostics subtrees removed BY SELECTOR. Four rules in it are load-bearing: **(0)** a lock surface's roster is INJECTED, because no mock scenario and no bench dongle can produce a locked device (every dialect answers unauthenticated), and an `open`/`unlocked` fixture MUST carry `router_admin.controls` — Configure is refused whenever no write was ever proven, only a LOCK overrides that refusal, so an `open` dongle without them cannot reach its own dialog at all. **(1)** the viewport is set BEFORE a surface is staged, never resized under an open dialog — `AppDialog` swaps a centred Dialog for a bottom Sheet at the breakpoint, so a mid-capture resize produces a shot belonging to neither. **(2)** The reachability probe scrolls to the viewport CENTRE (the `sticky top-0` header owns the top 56px, and `scrollIntoViewIfNeeded` counts an element beneath it as visible) and accepts any hit inside the SAME row (the shadcn button base carries `disabled:pointer-events-none`, so a disabled control structurally cannot answer a hit test and its fall-through lands on its own row — a dock or scrim is not inside the row and still reddens). **(3)** The fixture publishes `cellular_initializing: false` on EVERY frame, because the status merge preserves an omitted field and a raise-only flag would latch the initializing band over every later capture. Note that an element screenshot of a taller-than-viewport section RE-COMPOSITES the fixed mobile dock into each stitched tile, so the PNG shows it crossing the roster where nothing on screen does — that is a capture artifact, and the reachability probe is what settles it mechanically.
- **The C4 design-pass consolidation, and the one finding worth acting on [EXISTS]:** the Wave-3 UI pass closed with a consolidation run over the three e2e lanes on BOTH projects. `@a11y` (17 passed) and truthfulness (46 passed) were GREEN; `@visual` was RED at 137 passed / 21 failed / 138 skipped, and **none of the 21 is a Wave-3 visual regression** — 12 are goldens invalidated by shared-layout edits and never re-baselined, 5 are specs that predate PR #137's removal of `#encoder-source` and have been red since July, 2 are the known `sharing-diagnostics` pair, 1 needs the dev-only DevTools route, and one pair was flaky. Nothing was fixed and no golden re-baselined in that run, deliberately. **The root cause is the finding, not the 21:** CI runs NEITHER `@visual` NOR `@a11y` — `build-check.yml`'s only e2e job grep-inverts both, and no other workflow references them — so 50 visual specs and their committed golden PNGs are executed by no automated gate, which is precisely how the drift accumulated silently and how the July breakage stayed red unnoticed. Every entry is registered in [`../../docs/DESIGN-FOLLOWUPS.md`](../../docs/DESIGN-FOLLOWUPS.md) (DF-1…DF-13; the CI gap is DF-12, the highest-leverage one). Do NOT read a green PR as evidence a golden still matches, and do not re-baseline a golden to clear a red run without first establishing which of those categories it falls into.
...
- Unified update state machine + truthful failed-update UX (#184, #185) [EXISTS]: `UpdatesDialog.svelte` derives its Update button and in-progress state from the unified `update_state` machine, NOT the legacy `available_updates` field. **A start attempt now always ends somewhere visible (device-quality-wave2):** the dialog latches the outcome of the last `startUpdate` dispatch in its OWN `$state` rather than reading the async-op phase — that phase decays to `idle` after `ASYNC_OP_TERMINAL_LINGER_MS`, which is exactly how a refused start used to vanish. `osCommand` is called with `silent: true` (the sanctioned inline-band pattern) and the device's typed reason (`updates_disabled` / `streaming` / `already_updating` / `check_unavailable`, see `apps/backend/AGENTS.md` → SOFTWARE-UPDATE START CONTRACT) is rendered in a standing `update-start-refused` band; a start the device accepted but never reported progress for is called out by name, and `update_state.kind === 'success'` renders an explicit `update-succeeded` panel. **The full-screen `updating-overlay` is mounted globally by `Layout.svelte` off `status.updating` alone — it is trigger-agnostic and always was**, so Settings → Software Updates and the notification path show the identical percentage/phase/step counts; a live report that read as "the overlay only appears from the notification path" was really the start never firing. Locked by `Layout.updating-overlay.test.ts`. Do not make the overlay mount conditional on which surface started the update. The other legacy behaviour it replaced — a spec that injects only `available_updates` leaves `update_state` idle and the Update button never renders (both desktop update e2e specs now drive `update_state`). **The CHECK button reports that it ran (device-quality-wave2).** Clicking "Check for updates" used to change NOTHING observable for 11 s on a real board while the device's own log proved the check ran and succeeded in 1.8 s. The dialog cancelled its own spinner: it latched completion on "the state is no longer `checking`", and `checking` sits BELOW `available` in the state machine, so a device that already knows about an update never publishes a `checking` frame — the condition was already true on the first flush after the click. Completion is now latched on a NEW `checked_at` (stamped by every completed cycle regardless of which state it lands in), the up-to-date summary carries a `update-last-checked` line so a successful no-change check is distinguishable from a dead button, `check_failed` renders a typed `update-check-failed` band (`refresh_failed` / `discovery_failed`) INSTEAD of the old false "System is up to date", and a device that declines to run the check at all renders its typed reason in `update-check-refused`. Do NOT re-latch the spinner on a `checking` transition, and do NOT treat `check_failed` as `failed` — the latter is an install that ran and failed. Backend contract: `apps/backend/AGENTS.md` → SOFTWARE-UPDATE CHECK CONTRACT. Coverage: `UpdatesDialog.check.test.ts`.
  **…AND THE DIALOG NOW SAYS WHICH LAYER, WHETHER THE REPOSITORY WAS REACHED, AND WHY A START WAS REFUSED BEFORE APT RAN.** Three additions, each closing a place the dialog previously said less than the device knew.
  **(1) The package list carries its LAYER, and a kept-back package is named as uninstallable rather than omitted.** `update_state.available.packages[]` carries `{name, layer, kept_back?, actionable}` plus a sibling `actionable_count`. A `platform` entry renders the `settings.updates.layer.platformBand` band ("Ships with the next OS image") and a kept-back entry its own line — neither is hidden, because an operator watching a package sit in the list release after release needs to be told it is not this button's to install. **The Install control gates on `actionable_count`, never on `package_count`**, and the count is emitted even when it is `0`: a discovery that found only platform or kept-back packages is a real answer, and reading the wrong field is how the button came to offer an install the device would refuse.
  **(2) The reachability line names the family that worked.** `update_state`'s `reachability` block states BOTH families explicitly on every arm that carries it, so the dialog can say "reached over IPv4, no IPv6 route" instead of leaving an operator to guess why a check took a minute. Two new typed check failures render their own bands rather than the old false "System is up to date" — `repos_unreachable` and `captive_portal`, the latter naming the sign-in page and carrying NO URL (the portal's own address is not ours to advertise). Both resolve through `checkFailureMessage`'s switch, whose `default → undefined` arm is retained so a future wire reason degrades to the generic pointer instead of rendering its own dotted key. Device half: `apps/backend/AGENTS.md` → APT REACHES THE REPOSITORY OVER THE FAMILY THAT WORKS.
  **(3) `update_preflight_failed` is a TERMINAL state the dialog must settle on, even when it paints no progress.** A space-admission refusal can arrive in milliseconds, before any progress frame exists, so the pending-operation confirmation and the stale-start-outcome effects must both recognise a fast terminal — otherwise a refusal that never got a spinner reads later as a stalled "Applying…", which is the exact false stalled-start report this contract exists to prevent. All eleven `preflight_reason` literals have operator copy in all ten locales, the capacity sentence is distinct from every cannot-measure sentence, and a success may separately carry a non-fatal `cleanup_warning: post_clean_failed` band that never contradicts the success itself. Device half: `apps/backend/AGENTS.md` → …AND THE TRANSACTION IS ADMITTED AGAINST REAL FREE SPACE.
  **Board-evidence boundary, stated so it is not overread:** the backend's family selection was proven on an Orange Pi 5+ (real `apt-get` argv observed carrying `-o Acquire::ForceIPv4=true`, matching `{ipv4: ok, ipv6: no_route, used: ipv4}` frame). That drill deployed NO frontend, so the reachability LINE above has not been read off a board — the board was running a released `.deb` predating it. The Rock 5B+ leg of the same drill is DEFERRED on board reachability. Do not upgrade either to "hardware-proven".
...
- No manual UI primitive files in `lib/components/ui/` — use the shadcn-svelte CLI. That rule is what decided the `slider.svelte` warning cleanup: it was regenerated from the CLI, not patched, and the aria-forwarding hand-edit it dropped is a follow-up rather than a hand-edit to re-apply.
...
- Don't statically import a config dialog into a destination view — route it through `lazyDialog()` + `<LazyDialog>`, and don't replace the registry with an inline `{#await import(...)}` (that re-fetches on every open). Don't give `LazyDialogFallback` a `data-testid`, and don't drop its 150 ms delay. **The rule covers every mount site, not just the destination view**: a dialog is split only if NO module in the graph imports it statically, so one surviving static import silently neuters every `import()` of it elsewhere. `NetworkView` registered `HotspotDialog` lazily while `main/network/WifiSection.svelte` still imported it for its per-radio configurator, so the registration bought nothing for an entire effort — rolldown said `INEFFECTIVE_DYNAMIC_IMPORT` and then built successfully, so nothing failed. `src/tests/lazy-dialog-static-import-gate.test.ts` is the gate; its two exemptions are `main/dialogs/**` (a dialog composing its own sub-components) and `lib/federation/*-entry.ts` (a hosted bundle's graph must stay statically complete). A TYPE-only import is always fine — it creates no runtime edge.
...
- Don't restore `svelte-persistent-runes` as the `$persist` RUNTIME, and don't fold the two Vite configs' `resolve.alias` arrays back into the object map. CeraUI keeps the package's PREPROCESSOR (`svelte-persistent-runes/plugins`) and substitutes `src/lib/stores/persist-runtime.ts` for its runtime through an EXACT-match alias defined once in `vite.persist.ts`. The package's runtime entry re-exports eleven interchangeable serializer/storage presets — `next-json`, `superjson`, `devalue`, `esserializer`, `php-serialize`, `serialize-anything`, `@macfja/serializer`, `sjcl-es`, `sjcl-codec-hex`, `browser-cookies`, `deep-copy-all` — in a shape no bundler can shake: `dist/index.mjs` carries a BARE side-effect `import` of each, and `dist/options.mjs` instantiates every preset in ONE fused declaration whose first declarator IS the default the runtime uses, so one live declarator keeps every factory call and every factory call keeps its library. Measured on this tree: **427,866 raw / 78.1 KiB gzip** of the shipped SPA reachable by nothing, and `rollupOptions.treeshake.moduleSideEffects` recovers only 3.5 KiB of it because the fused declaration is a tree-shaking problem rather than a side-effect one. All seven `$persist` call sites pass exactly two arguments, so the resolved options are always the package's own defaults (`JSON.stringify`/`JSON.parse` over `window.localStorage`) and every preset is dead by construction. The substitution reproduces that default path exactly, including the parts that look accidental — `storageRead` collapsing `""` to `undefined`, `load` deserializing only a `string`, `save` ignoring `undefined`, and a caller-supplied `options` object still overriding per key. Vitest deliberately carries NO persist alias, so `src/tests/persist-runtime.test.ts` compares the substitute against the REAL package and an upstream change to `load`/`save` reddens the suite. The object alias map is a PREFIX match and would capture `.../plugins` (the preprocessor both configs import) and `.../options` (the preset registry a future call site is meant to reach explicitly) — hence the array form with an anchored regex.
...
- Don't add `[data-slot='switch']` to `app.css`'s touch-mode `min-height` list — growing a switch's BOX stretches the painted track into a 44px pill and strands the thumb, whose checked position is a translate off its own width. It reaches TT-1 through its `::after` hit area, and the a11y gate asserts both bounds so the one-line "fix" reddens. That override must also stay UNLAYERED: the inset it beats lives in `@layer utilities`, which outranks `@layer base` whatever the specificity.
...
- Don't render a refused net-mode catalog as `absent` — the firmware ANSWERED, and hiding the section reports a modem with no network mode rather than one whose firmware declined to say. `netModeSectionView` answers `blocked`, and the refusal arm renders no control because the chips are `children` and no `control` snippet is passed. Don't re-derive that view inside the `.svelte` either: it moved out so the claim is assertable without mounting a dialog.
- Don't ship a Wi-Fi or a restart control for a router dongle, under any gate. No provider in the pinned control package exposes either operation for any dialect, so there is nothing to gate — and the dongle's SSID being READABLE is not evidence it can be changed. Don't remove the unavailability rows to "clean up" either: an operator who came looking for the switch the vendor's own page has must be told this device will not offer one.
- Don't turn the LAN-subnet rewrite into a toggle, don't offer it where `router_admin.controls` is absent, and don't build its request anywhere but `subnetRewriteRequest` — the device's input schema is `.strict()` with `confirm: z.literal(true)`, so a hand-built request is one omission away from never reaching the handler. And don't render a `blocked` outcome as applied or refused: the dongle answered at NEITHER address, which is what the outcome band's `unknown` kind is for.
...
- Don't render a modem wire field verbatim because it "usually" holds a display string. `status.network_type` is `4G` on one modem and `hspa-plus` on the next, because `mmConvertAccessTech` passes an access technology it could not fold through unchanged — route it through `accessTechnologyDisplay`, and make sure the raw value has a diagnostics row to land in before you stop printing it. The same rule covers `network_type.supported` (`networkModeOperatorLabel`) and a dongle's own `vocabulary` detail fields. `src/tests/modem-raw-token-sweep.test.ts` is the gate: it collects each fixture's machine identifiers FROM THE PAYLOAD and asserts none reach operator copy on any of the three surfaces, so a new wire field is covered the moment a fixture states it.
- Don't point `isMachineIdentifier` at a value the OPERATOR chose (an SSID, a hotspot name, a saved APN) — it is a shape test for device vocabularies, and rerouting user text into diagnostics hides the operator's own setting from them.
...
- Don't apply an Ethernet port role on the radio selection, and don't give `EthernetRoleSelector` an RPC or an `osCommand` back. The role is staged dialog state and `NetifDialog.save()` is its only dispatch site — a role change reconfigures the port and can drop the LAN path the operator is reading the page over, so one stray tap on the kiosk touchscreen must not be able to cause it. Don't re-add the arm/confirm two-step either: Save is the confirmation gate now, so the streaming interlock ESCALATES the staged band instead of stacking a second confirm on top of it. And don't drop the staged band or let it render when the staged role equals the applied one — a picked rung looks exactly like the rung the device is already on, which is the whole reason the band exists.
...
- Don't gate the embedded-audio surface on `capabilities.network_embedded_audio`. That capability answers whether the ENGINE can ROUTE the muxed track; the picker asks whether the SOURCE owns a card, and an rtmp/srt row owns none under either answer. It is also absent from the engine-starting and engine-unavailable fallback snapshots, so gating on it handed the operator a list of unrelated microphones on every engine blip. `SourceSection` and `AudioDialog` read `audioKind === 'embedded'` alone and must stay in lockstep; the routing question keeps its own surface (IdleCockpit's roadmap pill).
...
- Don't show the "reconnecting" banner on the raw `isConnected` edge, and don't extend its grace period to the `rebooting` or `failed` treatments. Don't re-stamp `disconnectedSince` on each retry (that restarts the window forever), don't call `Date.now()` inside `deriveConnectionUx`/`reduceConnection`/`deriveOfflinePageVisible` (time is injected), and don't drive the re-derivation with a raw `setInterval` — `createStalenessClock` + `shouldRunBannerGraceClock` runs only inside the window and stops itself.
- Don't let `offline-state.svelte.ts` decide for itself when the full-page takeover renders, and don't give it a second grace constant. It records a REQUEST; `getShouldShowOfflinePage()` folds that through the ONE shared verdict. A `handleOffline()` that assigns the takeover directly is the exact regression this replaced — the loudest surface in the app was the only undebounced one, and on a PWA it spent a `window.location.reload()` on blips nobody was ever shown. Don't arm the recovery poll on the raw edge either, and don't re-compose the raw request at a render site: `App.svelte` and `DisconnectedBanner` read the same getter Layout does, so a Layout-only fix leaves both of them raw.
- Don't add a toast for a connection loss. `PWAStatus`'s band is mounted unconditionally and already states it, from the same boolean, at the same instant — a toast there is a second signal for one fact, on the overlay layer, and `showConnectionLostToast` was removed for exactly that. A band is for an ongoing state; a toast is for a transition that would otherwise be missed.
...
- Don't render a Versions row's raw producer string — route it through `splitVersionValue` (`lib/system/version-display.ts`), which promotes the version and demotes build metadata to a secondary line. The rows come from five unrelated producers, and TWO now carry a `(...)` tail: `srtla_send -v`'s `(branch@hash) [package]`, and CeraUI's own `<CalVer> (<commit>)`, which the backend composes into that SAME shape precisely so the commit is demoted with no wire change and no row-specific branch here (see `apps/backend/AGENTS.md` → …AND A BUILD-TIME VERSION IS NOT A COMMIT). Don't "fix" CeraUI's row by adding a second wire field or a per-row renderer for it. And don't drop `VersionsDialog`'s open-time `rpc.system.getRevisions()` re-pull: the login-time push is a snapshot, and cerastream can come up or be upgraded after login (see `apps/backend/AGENTS.md` → A VERSION AN OPERATOR READS MUST BE A LIVE READ).
