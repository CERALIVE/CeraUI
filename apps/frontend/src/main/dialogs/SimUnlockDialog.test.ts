...
const submitButton = () => screen.getByTestId('sim-pin-submit') as HTMLButtonElement;
...
describe('SimUnlockDialog — PIN entry (Task 23)', () => {
	it('shows the PIN field + submit for a SIM-PIN-locked modem, submit gated on a valid PIN', () => {
		render(SimUnlockDialog, {
			props: { open: true, deviceId: '0', modem: makeModem({ required: 'sim-pin', remainingAttempts: 3 }) },
		});
	...
	it('submits the PIN via unlockSimPin and closes on success', async () => {
		unlockSimPin.mockResolvedValue({ state: 'success' } satisfies SimUnlockOutput);

		render(SimUnlockDialog, {
			props: { open: true, deviceId: '2', modem: makeModem({ required: 'sim-pin', remainingAttempts: 3 }) },
		});
		...
		await waitFor(() => expect(screen.queryByTestId('sim-pin-input')).toBeNull());
	});

	it('surfaces the remaining attempts on a wrong PIN without auto-resubmitting', async () => {
		unlockSimPin.mockResolvedValue({ state: 'wrong-pin', remainingAttempts: 2 } satisfies SimUnlockOutput);
	...
	it('shows the PUK-required state with no PIN field for a PUK-locked SIM', () => {
		render(SimUnlockDialog, {
			props: { open: true, deviceId: '0', modem: makeModem({ required: 'sim-puk' }) },
		});
