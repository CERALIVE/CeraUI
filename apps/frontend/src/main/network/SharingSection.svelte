<!--
  ...
  QUIET WHEN NOTHING IS SHARED. With no client zone (`view.quiet`) the card is
  one unframed hint row and nothing else — uplink health, client zones, the
  diagnostics disclosure and the shaping priority inside it all measure a path
  no traffic takes, and stacking four instruments under "Sharing is off" reads
  as a fault report about a feature nobody switched on. The card itself NEVER
  goes away: the hint row is how an operator discovers sharing exists, and it
  states what to turn on. Everything returns the moment a zone appears.
-->  
<script lang="ts">
...
>
	...
	<div
		class={cn(
			...
			// Quiet: nothing follows it, so the hint row carries no surface, no
			// border and no separating margin — it IS the card's body.
			!view.quiet &&
				cn('mb-3 rounded-xl border p-3', TONE_SURFACE[view.headline.tone]),
		)}
