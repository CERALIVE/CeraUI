...
export function autoSelectNextOption(
	...
	const availableEncoders = Object.keys(groupedPipelines[properties.inputMode] || {});
	console.log(`🔍 Available encoders for ${properties.inputMode}:`, availableEncoders);

	if (properties.encoder && !availableEncoders.includes(properties.encoder)) {
		console.log(`❌ Encoder '${properties.encoder}' invalid for inputMode, clearing all dependents`);
	...
	if (!result.encoder) {
		console.log('⏸️ Multiple encoder options - user must choose, clearing dependents');
		result.resolution = undefined;
		result.framerate = undefined;
		result.pipeline = undefined;
		console.log('🧹 DEPENDENT CLEARING - resolution, framerate, pipeline set to undefined');
		return result;
	}
	...
	console.log(`🔍 Available resolutions for ${properties.inputMode}/${result.encoder}:`, availableResolutions);

	if (result.resolution && !availableResolutions.includes(result.resolution)) {
		console.log(`❌ Resolution '${result.resolution}' invalid for encoder, clearing dependents`);
		result.resolution = undefined;
		result.framerate = undefined;
		result.pipeline = undefined;
	...
	if (!result.resolution && availableResolutions.length === 1) {
		console.log(`🎯 Auto-selecting single resolution: ${availableResolutions[0]}`);
		...
		result.framerate = undefined;
		result.pipeline = undefined;
		console.log('🧹 NO OPTIONS CLEARING - resolution dependents set to undefined');
		return result;
	}

	// If no resolution selected and multiple options, ensure dependents are cleared
	if (!result.resolution) {
		console.log('⏸️ Multiple resolution options - user must choose, clearing dependents');
		result.framerate = undefined;
		result.pipeline = undefined;
	...
	const availableFramerates = groupedPipelines[properties.inputMode][result.encoder][result.resolution];
	console.log(`🔍 Available framerates for ${properties.inputMode}/${result.encoder}/${result.resolution}:`,
		availableFramerates?.map(p => p.extraction.fps));

	if (result.framerate) {
		const frameratExists = availableFramerates?.some(p => p.extraction.fps === result.framerate);
		if (!frameratExists) {
			console.log(`❌ Framerate '${result.framerate}' invalid for resolution, clearing`);
			result.framerate = undefined;
			result.pipeline = undefined;
			console.log('🧹 FORCED CLEARING - framerate and pipeline set to undefined');
		}
	}
	...
	} else if (!result.framerate && (!availableFramerates || availableFramerates.length === 0)) {
		console.log('⚠️ No framerates available - stopping investigation');
		result.framerate = undefined;
		result.pipeline = undefined;
		console.log('🧹 NO OPTIONS CLEARING - framerate and pipeline set to undefined');
		return result;
	}

	// If no framerate selected and multiple options, ensure pipeline is cleared
	if (!result.framerate) {
		console.log('⏸️ Multiple framerate options - user must choose, clearing pipeline');
		result.pipeline = undefined;
	...
	if (result.encoder && result.resolution && result.framerate) {
		const foundPipeline = availableFramerates?.find(p => p.extraction.fps === result.framerate)?.identifier;
		if (foundPipeline) {
