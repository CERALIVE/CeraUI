...
describe('shouldReconcileOnReconnect — reconnect-edge gate (Task 29)', () => {
	it('reconciles on the reconnect edge (→ connected) while a toggle is pending', () => {
		expect(shouldReconcileOnReconnect('disconnected', 'connected', true)).toBe(true);
		expect(shouldReconcileOnReconnect('connecting', 'connected', true)).toBe(true);
		expect(shouldReconcileOnReconnect('error', 'connected', true)).toBe(true);
	});

	it('does NOT reconcile when nothing is in flight', () => {
		expect(shouldReconcileOnReconnect('disconnected', 'connected', false)).toBe(false);
		expect(shouldReconcileOnReconnect('connecting', 'connected', false)).toBe(false);
	});
	...
	it('does NOT reconcile on a steady connected→connected tick (no edge)', () => {
		...
		expect(shouldReconcileOnReconnect('connected', 'connected', true)).toBe(false);
	});

	it('does NOT reconcile on the disconnect edge (→ not connected)', () => {
		expect(shouldReconcileOnReconnect('connected', 'disconnected', true)).toBe(false);
		expect(shouldReconcileOnReconnect('connected', 'connecting', true)).toBe(false);
