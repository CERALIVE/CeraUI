...
Bun/TypeScript HTTP + WebSocket server. Serves the frontend static bundle, exposes all device control via oRPC over WebSocket, drives the `cerastream` engine over structured IPC (`@ceralive/cerastream` public-npm registry dep) and `srtla-send-rs` via the private workspace package `@ceraui/srtla-send` (`packages/srtla-send/`).
...
- Don't import from `@ceralive/srtla` — that package is retired from CeraUI. Use `@ceraui/srtla-send` (the sender binding, now a private workspace package at `packages/srtla-send/`, never published). Check `../../../srtla-send-rs/AGENTS.md` before touching call sites — the binary's CLI/telemetry contract still lives there.
