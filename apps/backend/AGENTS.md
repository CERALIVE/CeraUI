...
- **It is hysteresis on the VERDICT, not on the sampling.** Distinct from every
  ...
  is believed.
- **`CAPTURE_ABSENCE_GRACE_MS` is 2 000 ms**, sized against BOTH halves of the
  measurement (the ≈400 ms engine hole and the 2.0 s re-registration ceiling). It
  is deliberately not larger: it sits under `AUDIO_METER_FRAME_ABSENCE_MS`
  (2 500 ms), so the grace can never outlive the watchdog that recovers a
  genuinely dead feed, and a real unplug reaches the operator within one meter
  frame of the window closing.
...
- **"Is the row there" is NOT the question.** Three things count as degraded: the
  row is absent, the row is `lost`, or the row is PRESENT but has lost the
  `physical_group_id` it was carrying a moment ago. The third is the one the
  window actually produces on this board — when the `/dev` scan sees the node
  return first, the hotplug merge restores the row through
  `withKnownEngineMetadata`, which restores durable IDENTITY (`kind`/`stable_id`)
  and deliberately refuses to re-assert a same-moment topology relation. That
  refusal is correct and must not be "fixed"; the row is simply useless to rule 5,
  and the grace is what covers it.
- **Bounded and self-clearing, with no renewal path.** After
  `CAPTURE_ABSENCE_GRACE_MS` of UNINTERRUPTED degradation the memory is dropped
  and the live view is reported verbatim — a true unplug reads exactly as it did
  before this module existed. Only a genuinely healthy observation resets the
  run, so polling the window at the meter's 5 Hz cadence extends nothing.
