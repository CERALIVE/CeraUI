...
describe("modem-control compatibility projections", () => {
	...
	test("the EXACT 1.4.0 pin imports every projection", async () => {
		...
		// modem-stack v1.4.0 moves Debian builds to Trixie; control/ changes only
		// its package version, not the projection API. Release change record:
		// https://github.com/CERALIVE/modem-stack/commit/9a4ff88ed1059aae65ef0608cdba6620ed07c3fb
		const packageJson = await Bun.file(
			join(BACKEND_ROOT, "..", "package.json"),
		).json();
		expect(packageJson.dependencies["@ceralive/modem-control"]).toBe("1.4.0");
		for (const module of MIGRATED_MODULES) {
