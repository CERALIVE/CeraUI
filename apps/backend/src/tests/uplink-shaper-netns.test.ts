 ...
 * `uplink-steering-netns.test.ts`. Both netns-semantics tests share one
 * dedicated CI step so they run with CPU headroom rather than inside the full
 * backend suite; the assertions are unchanged, only WHEN they run.
 */
const NETNS_ISOLATED = process.env.CERALIVE_NETNS_ISOLATED === "1";

describe.skipIf(!NETNS_ISOLATED)(
	"uplink shaper — kernel netns semantics",
	() => {
