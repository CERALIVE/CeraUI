...
const pendingSchema = z.array(
	z.object({ name: packageName, version: version.optional() }),
);
export type PendingPackage = z.infer<typeof pendingSchema>[number];
const EMPTY: QuarantineDocument = {
	schema: 1,
	packages: [],
	os: [],
	failedCommits: [],
...
export class UpdateQuarantine {
	constructor(
		...
		private readonly writePins: (
			text: string,
		) => Promise<void> = writeQuarantinePins,
	) {}
	...
	async savePending(packages: readonly PendingPackage[]): Promise<void> {
		...
		writeFileAtomicSync(
			this.pendingPath(),
			JSON.stringify(pendingSchema.parse(packages)),
		);
	}
	...
	async clearPending(): Promise<void> {
		if (await Bun.file(this.pendingPath()).exists())
			await Bun.file(this.pendingPath()).delete();
	}

	private pendingPath(): string {
		return this.path === QUARANTINE_FILE
			? PENDING_FILE
			: `${this.path}.pending`;
	}

	async recordPackageFailure(
		packages: readonly QuarantinedPackage[],
		id = "unknown-commit",
		reason = "apt-exit-nonzero",
	): Promise<void> {
		const doc = await this.read();
		const next = quarantineSchema.parse({
			...doc,
			packages: [
				...doc.packages,
				...packages.filter(
			...
			failedCommits: doc.failedCommits.some((item) => item.id === id)
				? doc.failedCommits
				: [...doc.failedCommits, { id, reason }],
		});
		await this.pin(next.packages); // fail closed before recording a version as protected
		await this.persist(next);
	}

	async recordOsRollback(
		expectedVersion: string,
		bootedVersion: string,
	): Promise<void> {
		const doc = await this.read();
		if (
			expectedVersion === bootedVersion ||
			doc.os.some((item) => item.version === expectedVersion)
		)
			return;
		await this.persist({
			...doc,
			os: [...doc.os, { version: expectedVersion, bootedVersion }],
		});
	}
	...
	async reconcileCandidates(
		...
		newer: (candidate: string, bad: string) => Promise<boolean> = async (
			candidate,
			bad,
		) => {
			const result = await spawnWithTimeout(
				["dpkg", "--compare-versions", candidate, "gt", bad],
				{ timeoutMs: 10_000 },
			);
		...
		for (const bad of doc.packages) {
			...
			if (!candidate || !(await newer(candidate.version, bad.version)))
				kept.push(bad);
		}
