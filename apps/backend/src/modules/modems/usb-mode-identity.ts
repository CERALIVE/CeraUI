...
import { mmGetModemPorts } from "./mmcli.ts";
...
export interface ResolvedModemIdentity {
	...
	/** The FULL reported revision. The catalog certifies a PREFIX of it. */
...
export async function defaultResolveIdentity(
	...
	const ports = (await mmGetModemPorts(String(id))) ?? [];

	const devices = await createUsbEnumerator().enumerate();
	const device = devices.find((d) => d.ifname === ifname);
	if (device === undefined) return undefined;

	const stableKey = deriveModemStableKey(device.physicalUid);
	...
	if (stableKey === undefined || device.physicalUid === undefined) {
		return undefined;
	}
...
export async function confirmModemDataPath(ifname: string): Promise<boolean> {
	...
	while (Date.now() < deadline) {
		const state = (await nmDeviceProp(ifname, "GENERAL.STATE"))?.[0] ?? "";
		const address = (await nmDeviceProp(ifname, "IP4.ADDRESS"))?.[0] ?? "";
		// NetworkManager's `100 (connected)` is the registered-AND-activated state;
		// an address on top of it is the data path itself rather than a claim.
