...
Both backend and shared RPC pin companion cerastream bindings
**2026.9.10 / schema 0.20.0**. The regenerated `bun.lock` carries the registry
tarball's SHA-512 integrity. The skew and exact-registry gates remain unchanged.
No local link or fabricated integrity is committed.

The raw-classification surface this document describes arrived in the **earlier**
`2026.9.9 / schema 0.19.0` binding, published on 2026-09-18 from cerastream PR
#177's merge commit `c80e0ccc55c5ae5c0d1c7b75540efa8899ad543a`, tagged
`bindings-v2026.9.9`; its
[publish workflow](https://github.com/CERALIVE/cerastream/actions/runs/35395597683)
passed, including 181 binding tests, and npm reports that exact `gitHead`. That
is the minimum this consumer needs. Schema 0.20.0 keeps it verbatim and adds only
optional `active_encode` encode-telemetry fields, so the current pin satisfies it.
