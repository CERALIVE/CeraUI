...
import {wifiSignalSymbol} from "../wifi/wifi.ts";
/* Modem manager */
    ...
    for (const deviceId in msg) {
        ...
        if (isFullModemState && modemStatusUpdate.network_type) {
            ...
            updateOptionList(deviceCard.find('.network-type-input'),
                [options], modemStatusUpdate.network_type.active);
        }
        ...
        if (
            ...
            updateOptionList(networkSelect as JQuery<HTMLOptionElement>, [auto, options], selectedNetwork);
