...
// The reader is consumed as `@ceraui/srtla-send/telemetry`. These tests exercise
// it from a repo-local path — no `../`-escaping above the repo root, no sibling
// checkout. The golden fixture is this package's own copy of real Rust-producer
// output (ADR-001), kept beside the tests.
...
describe("ADR-002 cumulative bytes → survives the reader as a byte count", () => {
	...
	test("a producer predating ADR-002 still parses; the field reads undefined", () => {
		...
		expect(
			parsed.success && parsed.data.connections[0]?.bytes_sent_total,
		).toBeUndefined();
	});
...
describe("malformed input → graceful null (never an uncaught throw)", () => {
	...
	test("truncated JSON object returns null, does not throw", async () => {
		expect(
			await writeAndRead('{"schema_version":1,"last_updated_ms":1,'),
		).toBeNull();
	});
