...
const ja = {
	...
	settings: {
		...
		index: {
			...
			devicePasswordDesc: "ローカルネットワークでこのデバイスにアクセスするためのパスワードを設定します",
			...
			versions: "デバイスバージョン",
			versionsDesc: "インストール済みコンポーネントとファームウェアのバージョンを表示します",
		},

	dialogs: {
		enableInterface: "インターフェースを有効化",
		enableInterfaceDesc: "このインターフェースをボンディングストリーミングに使用します。",
		staticIp: "静的IPアドレス",
...
pairing: {
	...
	network: {
		...
		modem: {
			...
			noNetworksFound: "ネットワークがまだ見つかりません。スキャンして事業者を検索してください。",
			scanForNetworks: "ネットワークをスキャン",
			credentials: "認証情報",
			configureDescription: "セルラー接続設定を構成します",
			simUnlock: {
				title: "SIM PIN が必要です",
				description: "この SIM カードはロックされています。PIN を入力してモデムのロックを解除してください。",
				pinLabel: "SIM PIN",
	...
	live: {
		...
		configureToStart: "ストリーミングを開始するにはリレーサーバーを設定してください",
		editSettings: "設定を編集",
	...
	devtools: {
		...
		batteryLow: "バッテリー低下",
		batteryLowDesc: "デバイスのバッテリーが15%未満です。電源に接続してください。",
		socketPort: "ソケットポート",
		...
		testingTip1: "自動消去のタイミングをテストするため、さまざまな継続時間を試してください",
		testingTip2: "ボタンの操作を確認するためアクショントーストをテストしてください",
		testingTip3: "手動での消去をテストするため永続的なトーストを使用してください",
