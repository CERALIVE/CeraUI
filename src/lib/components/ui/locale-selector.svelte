...
let localeFlag = $derived.by(() => existingLocales.find(l => l.code === selectedLocale)!.flag);
...
<Select.Root type="single" value={selectedLocale} onValueChange={handleLocaleChange}>
  <Select.Trigger
    ...
    <div class="flex items-center gap-2">
      {#if localeFlag}
        <span class="text-lg" role="img" aria-label={localeName}>{localeFlag}</span>
      {:else}
        ...
        <Select.Item
          ...
          <div
            ...
            {#if localeOption.flag}
              <span role="img" aria-label={localeOption.name}>{localeOption.flag}</span>
            {:else}
              <Globe class={cn('h-3.5 w-3.5', isActive ? 'text-white' : 'text-gray-600 dark:text-gray-400')} />
            {/if}
          </div>

          <!-- Language Info -->
          <div class="flex-1">
            <div class={cn('font-medium', isActive ? 'text-white' : 'text-gray-900 dark:text-gray-100')}>
              {localeOption.name}
            </div>
            <div class={cn('text-xs', isActive ? 'text-white/80' : 'text-gray-500 dark:text-gray-400')}>
