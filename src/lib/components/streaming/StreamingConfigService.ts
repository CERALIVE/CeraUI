...
import type { ConfigMessage, PipelinesMessage } from '$lib/types/socket-messages';
...
export function buildStreamingConfig(properties: Properties, options: ConfigServiceOptions): ConfigMessage | null {
  ...
  if (!unparsedPipelines || !properties.pipeline) {
    console.warn('Cannot build streaming config: missing pipeline data or pipeline selection');
    return null;
  }

  const pipelineData = unparsedPipelines[properties.pipeline];
  if (!pipelineData) {
    console.warn('Cannot build streaming config: pipeline data not found for', properties.pipeline);
    return null;
  }
