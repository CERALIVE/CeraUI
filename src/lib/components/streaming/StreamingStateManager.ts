...
class StreamingStateManager {
  ...
  private _audioCodecsStore = writable<AudioCodecsMessage | undefined>(undefined);
  ...
  private _notAvailableAudioSourceStore = writable<string | undefined>(undefined);

  // Internal values for immediate access
  private _groupedPipelines: GroupedPipelines[keyof GroupedPipelines] | undefined = undefined;
  ...
  private processPipelines() {
    if (this._unparsedPipelines && get(_)) {
      ...
      const allGroupedPipelines = groupPipelinesByDeviceAndFormat(this._unparsedPipelines, {
      ...
      console.log(`[StreamingStateManager] Cannot process pipelines - missing data:`, {
        hasUnparsedPipelines: !!this._unparsedPipelines,
